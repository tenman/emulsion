...
<div class="article-wrapper <?php emulsion_template_identification_class( __FILE__ ) ?>">
	...
	<?php  
		wp_attachment_is_image( $emulsion_post_id ) ? emulsion_attachment_image( $emulsion_post_id, 'full', $excerpt ) : ''; 
	...
	?>
			<div class="attachment-metadata"><?php
				printf( '<div class="relate-post-title"><span class="label">%1$s : </span><a class="parent-entry-title" href="%3$s" rev="attachment">%2$s</a></div>', 
						...
						wp_kses_post( get_the_title( $post_info->post_parent ) ),
						esc_url( get_permalink( $post_info->post_parent ) )
						);

				empty($post_info->post_content) ? '': printf('<div class="description-text"><span class="label">%1$s : </span>%2$s</div>', 
																esc_html__('Description', 'emulsion'), wp_kses_post( $post_info->post_content ) );
			?></div>
	<?php
		}
