...
function emulsion_add_stylesheet() {
	...
	if ( 'active' !== get_theme_mod( 'emulsion_wp_scss_status' ) || is_admin() ) {
	...
	if ( is_child_theme() ) {
		...
		if ( file_exists( get_stylesheet_directory() . '/js/' . $emulsion_child_theme_slug . '.js' ) ) {
			 ...
			 */
			wp_register_script( $emulsion_child_theme_slug . '-js', 
					get_stylesheet_directory_uri() . '/js/' . $emulsion_child_theme_slug . '.js', 
...
if ( ! function_exists( 'emulsion_get_first_paragraph' ) ) {

	/**
	 * get first paragraph in html text
	 * @param type $content
	 * @return type
	 */
	function emulsion_get_first_paragraph( $content ) {

		if ( preg_match( '!<p[^>]*>(.+)?</p>!', $content, $regs ) ) {
...
if ( ! function_exists( 'emulsion_class_name_sanitize' ) ) {
	...
	/**
	 * Sanitize single class, space separated multiple classes, array classes.
	 * @param type $class
	 * @return type
	 */
...
if ( ! function_exists( 'emulsion_svg_icon' ) ) {
	...
	function emulsion_svg_icon( $args = array() ) {
		...
		/* $allowed_html = array(
		  'svg'	 => array( 'class' => array(), 'aria-labelledby' => array(), 'role' => array(), 'aria-hidden' => array(), 'width' => array(), 'height' => array() ),
		  'title'	 => array( 'id' => array(), 'desc' => array(), 'id' => array() ),
	...
	function emulsion_php_version_notice() {
		...
		printf( '<div class="%1$s"><p>%2$s<br />%3$s</p></div>',
			...
			sprintf(
				...
				esc_html__( 'Actual version is: %1$s, required version is: %2$s', 'emulsion' ),
				esc_html( PHP_VERSION ),
				esc_html( EMULSION_MIN_PHP_VERSION )
			)
		);
	}
}
