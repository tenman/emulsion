...
if ( ! function_exists( 'emulsion_classic' ) ) {
	...
	function emulsion_classic() {
		...
		if ( 'php' == emulsion_get_template_type( $template ) ) {
			wp_register_style(
					'emulsion-classic',
					get_template_directory_uri() . '/classic-templates/classic.css',
					$dependency_style,
		...
		if ( is_child_theme() ) {
			...
			$child_classic_url			 = get_stylesheet_directory_uri() . '/classic-templates/classic.css';
			$child_classic_path			 = get_stylesheet_directory() . '/classic-templates/classic.css';
 ...
 * @return type
 */
add_filter( 'template_include', 'emulsion_compatible_classic_template_include', 21 );
